...
import {
  deriveGameData,
  isEveryPlayerAnswerSubmitted,
} from "../../client/src/utils/game-utils";
import { GameStatus } from "../../client/src/types/game.types";

export const approveCurrentPrompt: ClientEventListeners["APPROVE_CURRENT_PROMPT"] =
  (gameId) => {
...
export const drawNewPrompt: ClientEventListeners["DRAW_NEW_PROMPT"] = (
  gameId,
  currentPromptId
) => {
...
export const editAnswer: ClientEventListeners["EDIT_ANSWER"] = (
  gameId,
  playerId,
  newAnswer
) => {
...
export const lockAnswer: ClientEventListeners["LOCK_ANSWER"] = (
  gameId,
  playerId
) => {
  const gameManager = GameManager.for(gameId);
...
export const moderateAnswerMark: ClientEventListeners["MODERATE_ANSWER_MARK"] =
  (gameId, playerId, mark) => {
...
export const pausePlayerTyping: ClientEventListeners["PAUSE_PLAYER_TYPING"] = (
  gameId,
  playerId
) => {
